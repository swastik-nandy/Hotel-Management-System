  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (rooms.length > 0) {
        const room = rooms[0];
        navigate(
          `/book/${room.id}?checkIn=${checkIn}&checkOut=${checkOut}&price=${totalPrice}&roomType=${selectedType}&branchId=${selectedBranch.id}`
        );
      } else {
      ...
      >
        ...
        <div className="relative z-10 max-w-6xl mx-auto text-center space-y-4">
          <h1
            ...
            data-aos="fade-down"
          >
            Discover <span className="text-[#a0765b] font-[Alex Brush]">Luxe</span> Rooms
          </h1>
          <p
            className="text-xl md:text-2xl text-[#7b5c47]"
            style={{ fontFamily: "New Times Roman, serif" }}
            data-aos="fade-up"
          >
            Elegance, comfort, and charm await your stay.
          </p>
          <div className="flex justify-center items-center gap-2" data-aos="fade-up">
            <div className="h-[2px] w-20 bg-[#a0765b]" />
            <div className="text-[#a0765b] text-2xl font-semibold">✦</div>
            <div className="h-[2px] w-20 bg-[#a0765b]" />
          </div>
        </div>

        {/* Filter Form */}
        <div className="bg-white/60 backdrop-blur-md rounded-3xl shadow-xl p-8 border border-[#e6ddd5]">
        ...
        <div className="text-center">
          <motion.button
            ...
            disabled={loading}
            className={`mt-6 px-10 py-3 rounded-full font-semibold tracking-widest transition-all duration-300
  border-2 border-[#5a4234] bg-transparent text-[#5a4234]
  backdrop-blur-md bg-white/20 shadow-md
  hover:bg-[#5a4234] hover:text-white ${
